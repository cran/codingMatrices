...
\item For higher way models the process of constructing the model
  ...
  matrix.  Thus each factor doubles the numbr of partitions, (or
...
fitted with such a design, however, requirs the \emph{contrast matrices}
...
\item By an interpretation of the regression coefficients $\bsb$ we mean
  relating them to the subclass means, $\bsm$, which have a naturl,
  unequivocal intrpretation.
\item For interpretative purposes, it is helpful to think of the
  ...
  \item In general, an ineraction of any order is got by averaging
...
Section~\vref{sec:examples} has a single respons, \texttt{Wt}, and two
...
\texttt{code\_*} funtions from the present package.
